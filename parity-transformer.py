...
class MaskedTokenAndPositionEmbedding(keras.layers.Layer):
    def __init__(self, maxlen, vocab_size, embed_dim):
        ...
        self.pos_emb = keras.layers.Embedding(input_dim=maxlen+1,output_dim=embed_dim,mask_zero=True)
...
def data_parity(lb,ub, dim):
	data_arr= []
	for k in range(dim):
		length =np.random.randint(lb, ub+1) #lb=lower bound and ub=upper bound
		string_bit =''.join(map(str, np.random.randint(0,2, size=length)))
	##loop through all the parities
		parity_string =""
		update =False
		for each_bit in string_bit:
			if each_bit == '1':
				update =not update
			else:
				parity_string +=str(int(update))
		data_arr.append([string_bit, parity_string])
	data1 =np.array(data_arr)
	shuffle = np.random.permutation(data1.shape[0])
	data= data1[shuffle,:]
	return data
data = data_parity(10, 30, 1000)
...
with strategy.scope():
    history = coupled.fit([encX,decX], decY,
          epochs=epochs,
...
with strategy.scope():
    preNTF = decX[-200:].copy()
    context = encoder.predict(encX[-200:])
    for i in range(decX[-200:].shape[1]-1):
        postNTF = decoder.predict([preNTF,context])
        preNTF[:,i+1] = postNTF[:,i,:].argmax(-1)
    predictions = postNTF
    print(float(MaskedSparseCategoricalAccuracy(decY[-200:],predictions)))
